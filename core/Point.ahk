...
class Point {
    ...
    refresh_pos() {
        ; while (not Game.is_game_process_exist()) || (Game.game_size.width == 0 || Game.game_size.height == 0) {
        while (not Game.is_game_window_exist()) {
            Sleep(500)
            Game.get_game_pos()
        }
        game_width := Game.game_size.width
        game_height := Game.game_size.height
